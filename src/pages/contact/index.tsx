...
const ContactPage = () => {
  ...
  return (
    <main className=" text-white min-h-screen py-16  h-auto my-28">
      <section className="h-[50vh]  relative flex justify-center items-center">
        ...
        <div className="relative flex justify-center items-center  ">
          ...
          <span className="absolute -bottom-5 -right-2 text-white text-4xl">
            ✦
          </span>
          <span className="absolute bottom-2 -right-10 text-white text-6xl">
            ✦✦
          </span>
        </div>
      </section>

      <section className="flex flex-col md:flex-row items-stretch justify-center p-12 h-auto">
  {/* Left Side (Form) */}
  <div className="flex-1 p-12 bg-gradient-to-br from-purple-900 to-gray-900 rounded-l-2xl min-h-[400px] flex flex-col justify-center">
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      {["name", "email", "phone", "company"].map((field) => (
        <div key={field}>
          ...
          />
          {errors[field] && (
            <p className="text-red-500 text-[8px] mt-1">{errors[field]}</p>
          )}
        </div>
      ))}
      <div>
  ...
  <div className="flex-1 p-12 flex flex-col space-y-4 bg-black border-2 border-purple-700 rounded-r-2xl justify-center min-h-[400px]">
    <h3 className="text-4xl font-bold mb-4 text-white">Our Address</h3>
    <div className="text-gray-300 space-y-2">
      <p className="flex items-center gap-2 text-lg">
        <FaMapMarkerAlt className="text-blue-400" /> {SOCIAL_LINKS.ADDRESS}
      </p>
      <p className="flex items-center gap-2 text-lg">
